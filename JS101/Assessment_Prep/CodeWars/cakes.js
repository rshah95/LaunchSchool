...
function cakes(recipe, ownedIngredients) {
  let neededIngredients = Object.keys(recipe);
  for (let idx = 0; idx < neededIngredients.length; idx++) {
    if (!Object.keys(ownedIngredients).includes(neededIngredients[idx])) return 0;
  }

  let maxRecipes = [];
  for (let ingredient in recipe) {
    let max = Math.floor(ownedIngredients[ingredient] / recipe[ingredient]);
    maxRecipes.push(max);
  }

  return Math.min(...maxRecipes);
}
